...
export class TaskListComponent implements OnInit {
  ...
  constructor(private taskService: TaskService, private router: Router, private route: ActivatedRoute, private store: Store<AppState>, public authenticationService: AuthenticationService) {}
  ...
  searchTasks(): void {
    this.tasks$ = this.tasks$.pipe(
      map(tasks => tasks.filter(task =>
        ...
        || (task.assignedTo && task.assignedTo.some(username =>
    ...
    this.tasks$ = this.tasks$.pipe(
      map(tasks => [...tasks].sort((a, b) => {
        ...
        if (typeof valueA === 'string' && typeof valueB === 'string') {
          ...
          return asc ? valueA.getTime() - valueB.getTime() : valueB.getTime() - valueA.getTime();
        } else {
          return 0;
        }
      }))
